                ...
                <div class="container">
                    ...
                    <div class="collapse navbar-collapse" id="navbarNav">
                        <ul class="navbar-nav ms-lg-5 me-lg-auto">
                            ...
                            <li class="nav-item">
                                <a class="nav-link click-scroll" href="Profile.php"> User Profile</a>
...
<?php
$updateQueryCapacity = "UPDATE programs SET current_capacity = current_capacity - 1 WHERE program_id = ?";
$stmtCapacity = $conn->prepare($updateQueryCapacity);
$stmtCapacity->bind_param("i", $programID);

if (!$stmtCapacity) {
    echo "Error preparing update statement: " . $conn->error;
} else {
    if ($stmtCapacity->execute()) {
        // Insert application into applications table
        $insertQueryApplication = "INSERT INTO applications (user_id, program_id, application_date) VALUES (?, ?, NOW())";
        $stmtApplication = $conn->prepare($insertQueryApplication);
        $stmtApplication->bind_param("ii", $userID, $programID);

        if ($stmtApplication->execute()) {
            // Application successful, display success message or redirect
            echo "Application submitted successfully!";
        } else {
            echo "Error submitting application: " . $stmtApplication->error;
        }

        $stmtApplication->close();
    } else {
        echo "Error updating capacity: " . $stmtCapacity->error;
    }
